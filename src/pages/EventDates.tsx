...
export default function EventDates() {
  ...
  const handleWhatsApp = () => {
    if (!selectedDate || !selectedTime || !eventType.trim() || !guestCount.trim()) return;
    const dateFormatted = format(selectedDate, "dd/MM/yyyy (EEEE)", { locale: ptBR });
    const msg = `Olá! Gostaria de verificar a disponibilidade da seguinte data para evento no Espaço Lamoniê:\n\n📅 Data escolhida: ${dateFormatted}\n🕐 Horário do evento: ${selectedTime}\n🎉 Tipo de evento: ${eventType.trim()}\n👥 Quantidade de pessoas: ${guestCount.trim()}\n\nPoderia confirmar se essa data está disponível?`;
          ...
          <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white rounded-3xl shadow-xl shadow-emerald-500/5 border border-emerald-100 overflow-hidden">
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 px-6 pt-8 pb-6 text-center">
                ...
                <h2 className="text-xl font-bold text-stone-800">Solicitação enviada!</h2>
                <p className="text-sm text-stone-500 mt-1">
                  Sua mensagem foi aberta no WhatsApp. Aguarde nosso retorno.
                </p>
              </div>
              <div className="px-6 py-5 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-stone-600">{selectedTime}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-stone-50 flex items-center justify-center">
                    <PartyPopper className="w-4 h-4 text-[#1F4D3A]" />
                  </div>
                  <span className="text-sm text-stone-600">{eventType}</span>
                </div>
              </div>
            </div>
            <button
              ...
              className="w-full text-center text-sm text-[#1F4D3A] font-semibold py-3 hover:underline"
            >
                ...
                <div className="px-5 pb-5 space-y-3.5">
                  <div>
                    ...
                    <div className="relative mt-1.5">
                      <PartyPopper className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-300" />
                      <Input
                        id="eventType"
                        value={eventType}
